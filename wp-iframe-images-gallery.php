...
function iframe_install() 
{
	...
	if($wpdb->get_var("show tables like '". WP_iframe_TABLE . "'") != WP_iframe_TABLE) 
	{
		...
		for($i=1; $i<=4; $i++)
		{
			$sSql = $IsSql . " VALUES ('".get_option('siteurl')."/wp-content/plugins/wp-iframe-images-gallery/images/250x167_$i.jpg', '#', '_blank', 'Enter alt text here', '$i', 'YES', 'Group1', '0000-00-00 00:00:00');";
			$wpdb->query($sSql);
		}
	}
}
...
<ul>
    <li>Check official website for info <a href="http://www.gopipulse.com/work/2011/07/24/wordpress-plugin-wp-iframe-images-gallery/" target="_blank">click here.</a></li>
</ul>
</div>
<?php
}
